...
struct L2_Vectorized
{
  ...
  inline ResultType operator()(Iterator1 a, Iterator2 b, size_t size) const
  {
    ...
    while (a < lastgroup) 
	{
		diff0 = static_cast<ResultType>(a[0] - b[0]);
		diff1 = static_cast<ResultType>(a[1] - b[1]);
		diff2 = static_cast<ResultType>(a[2] - b[2]);
		diff3 = static_cast<ResultType>(a[3] - b[3]);
    ...
    while (a < last) 
	{
		diff0 = static_cast<ResultType>(*a++ - *b++);
		result += diff0 * diff0;
    }
    return result;
  }
