  ...
  Hash_Map<PairValueType, PairValueType> & _reindexBackward)
{
        ...
        iter != pairs.end(); ++iter)
  {
    if (_reindexForward.find(iter->first) == _reindexForward.end())
    {
	  ...
	  _reindexForward[iter->first] = static_cast<PairValueType>(dist);
      _reindexBackward[dist] = static_cast<PairValueType>(iter->first);
    }
    if (_reindexForward.find(iter->second) == _reindexForward.end())
    {
	  ...
	  _reindexForward[iter->second] = static_cast<PairValueType>(dist);
	  _reindexBackward[dist] = static_cast<PairValueType>(iter->second);
    }
  }
}
