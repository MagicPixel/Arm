...
Vec3 LatLonToEcef(const Vec3& lla)
{
	Vec3 ret;
	lla_to_ecef(lla[0], lla[1], lla[2], ret[0], ret[1], ret[2]);
	return ret;
}
class WalleIO
{
public:
	...
	bool ExportBlocks(const std::string& result_root, const std::vector<SfM_Data>& blocks, ESfM_Data flags_part)
	{
		...
		{
			sprintf(filename, "%08d", i);//blocks[i].block_id);
			...
			ExportPoseWithRMS(blocks[i], result_root + "/" + std::string(filename) + "_rms.txt ");
		}
	...
	bool ImportCameras()
	{
		...
		std::string camera_root = _sfm_data.s_root_path + "/calibration/intrinsics";
		std::string image_root = _sfm_data.s_root_path + "/images";
	...
	bool ImportMatches()
	{
		...
		for (std::size_t i = 0; i < files.size(); i++)
		{
			...
			stl::split(filename, "-And-", imagenames);
	...
	bool ImportGNSS(SfM_Data& sfm_data)
	{
		...
		for (Views::iterator it = sfm_data.views.begin(); it != sfm_data.views.end(); ++it)
		{
			...
			std::string filepath = sfm_data.s_root_path + "/gnss/" + imagename + ".gnss";
			geometry::Pose3 pose;
			import_pose(filepath.c_str(), pose);

			pose.center() = LatLonToEcef(pose.center());
	...
	bool ExportGNSSRms(const SfM_Data& sfm_data, const std::string& filename)
	{
		std::ofstream ofstr(filename.c_str());
		ofstr.setf(std::ios::fixed, std::ios::floatfield);  // 设定为 fixed 模式，以小数点表示浮点数
		ofstr.precision(3);
		...
		for (Poses::iterator it = poses.begin(); it != poses.end(); ++it)
		{
			...
			Mat3 Rsrc = it->second.rotation();
			Mat3 Rdst = gnss[pose_id].rotation();
			Vec3 angle1, angle2;
			ceres::RotationMatrixToAngleAxis((const double*)Rsrc.data(), angle1.data());
			ceres::RotationMatrixToAngleAxis((const double*)Rdst.data(), angle2.data());

			Vec3 center = it->second.center() - gnss[pose_id].center();

			Vec3 angle = (angle1 - angle2) * 180.0 / PI;
			ofstr << imagename << "\t" << center[0] << "\t" << center[1] << "\t" << center[2] << "\t" << NormL2<Vec3>(center) << "\t";
			ofstr << angle[0] << "\t" << angle[1] << "\t" << angle[2] << "\t" << NormL2<Vec3>(angle) << std::endl;
		}
	...
	bool ExportPoseWithRMS(const SfM_Data& sfm_data, const std::string& filename)
	{
		std::ofstream ofstr(filename.c_str());
		ofstr.setf(std::ios::fixed, std::ios::floatfield);  // 设定为 fixed 模式，以小数点表示浮点数
		ofstr.precision(3);
		...
		//imagename x	y	z	r[9] dX dY dZ  d
		ofstr << "#imagename\t" << "X\tY\tZ\t" << "R11\tR12\tR13\tR21\tR22\tR23\tR31\tR32\tR33\t" << "dX\tdY\tdZ\tRMS\t" << "dRx\tdRy\tdRz\tRMS" << std::endl;
		for (Poses::iterator it = poses.begin(); it != poses.end(); ++it)
		{
			std::string imagename = get_imagename(views[it->first]);
			IndexT pose_id = views[it->first]->id_pose;
			...
			Vec3 center = it->second.center();
			Vec3 dcenter = center - gnss[pose_id].center();
			Vec3 angle = (angle1 - angle2) * 180.0 / PI;

			ofstr << imagename << "\t" << center[0] << "\t" << center[1] << "\t" << center[2] << "\t";
			ofstr << Rsrc(0, 0) << "\t" << Rsrc(0, 1) << "\t" << Rsrc(0, 2) << "\t";
			ofstr << Rsrc(1, 0) << "\t" << Rsrc(1, 1) << "\t" << Rsrc(1, 2) << "\t";
			ofstr << Rsrc(2, 0) << "\t" << Rsrc(2, 1) << "\t" << Rsrc(2, 2) << "\t";
			ofstr << dcenter(0) << "\t" << dcenter(1) << "\t" << dcenter(2) << "\t" << NormL2<Vec3>(dcenter) << "\t";
			ofstr << angle[0] << "\t" << angle[1] << "\t" << angle[2] << "\t" << NormL2<Vec3>(angle) << std::endl;
		}
