...
namespace openMVG {
	namespace sfm {
		...
		public:
			...
			bool ProcessGNSS(SfM_Data& sfm_data, const std::vector<IndexT>& images);

			Pair MaxDistance(SfM_Data& sfm_data);

			void CheckGNSS(SfM_Data& sfm_data);

			void DiffPoses(SfM_Data& sfm_data, Poses& results, double scale);
			...
			bool calculate(const std::vector<Pose>& src, const std::vector<Pose>& dst, Mat33& R, Vec3& t, double& s);
			...
			double cal_scale(const Points3f& src, const Points3f& dst);

			Mat3 cal_rotation(const std::vector<Pose>& src, const std::vector<Pose>& dst);

			Vec3 cal_center(const Points3f& src);
			...
			void get_max_trangle(const Points3f& pts, std::size_t& a, std::size_t& b, std::size_t& c);
