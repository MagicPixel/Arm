...
public:
  ...
  bool Set_configuration_preset(EDESCRIBER_PRESET preset)
  {
    ...
    break;
    case HIGH_PRESET:
      _params._options.fThreshold = AKAZEConfig().fThreshold/10.0f;
    break;
    case ULTRA_PRESET:
     _params._options.fThreshold = AKAZEConfig().fThreshold/100.0f;
    break;
    default:
      return false;
    }
      ...
      case AKAZE_MSURF:
      {
        ...
        for (int i = 0; i < static_cast<int>(kpts.size()); ++i)
        {
          ...
          if (mask)
          {
			...
			if (maskIma(static_cast<int>(ptAkaze.y), static_cast<int>(ptAkaze.x)) > 0)
			{
				continue;
			}
          }
      ...
      case AKAZE_LIOP:
      {
        ...
        for (int i = 0; i < static_cast<int>(kpts.size()); ++i)
        {
          ...
          if (mask)
          {
            const image::Image<unsigned char> & maskIma = *mask;
			if (maskIma(
				static_cast<int>(ptAkaze.y),
				static_cast<int>(ptAkaze.x)) > 0)
			{
				continue;
			}
          }
          ...
          const SIOPointFeature fp =
              ...
              ptAkaze.size/2.0f, ptAkaze.angle);
